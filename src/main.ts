...
const conversionBase = document.getElementById(
  "conversion-base"
...
let baseCurrencyValue: string = "USD";

const fetchWrapper = new FetchWrapper("https://v6.exchangerate-api.com/v6/");
const apiKey: string = "9ad860587c1970717b5dfad2";

fetchWrapper.get(`${apiKey}/latest/${baseCurrencyValue}`).then((data: Data) => {
  conversionRates = Object.keys(data.conversion_rates).map((key) => [
    key,
...
function performCurrencyConversion() {
  ...
  let conversionRate: [string, number] | undefined;
  ...
  if (conversionRate) {
    conversionResult.textContent = `  ${conversionRate[1]} ${targetCurrencyValue}`;
  }
}
